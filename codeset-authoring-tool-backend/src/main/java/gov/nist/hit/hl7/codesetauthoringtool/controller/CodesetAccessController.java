...
public class CodesetAccessController {
    ...
    public ResponseEntity<CodesetAccessDTO> getCodeset(@PathVariable String id, @RequestParam(name = "version", required = false) String version,
                                                       @RequestParam(name = "match", required = false) String match) throws IOException {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        CodesetAccessDTO codeset = codesetAccessService.getCodeset(id, version, match);
